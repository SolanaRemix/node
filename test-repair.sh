...
if echo "$CLONE_RESPONSE" | jq -e '.success' > /dev/null; then
...
FIX_COUNT=$(echo "$REPAIR_RESPONSE" | jq -r '.fixes | length')
if echo "$REPAIR_RESPONSE" | jq -e '.success' > /dev/null; then
    ...
    if [ "$FIX_COUNT" -gt 0 ]; then
        ...
        echo "$REPAIR_RESPONSE" | jq -r '.fixes[]' | sed 's/^/    • /'
    fi
    # Show blockchain hash if present
...
HAS_CHANGES=$(echo "$REPAIR_RESPONSE" | jq -r '.hasChanges')
if [ "$HAS_CHANGES" = "true" ]; then
    ...
    if echo "$COMMIT_RESPONSE" | jq -e '.success' > /dev/null; then
...
if [ "$HAS_CHANGES" = "true" ]; then
    ...
    PR_NUMBER=$(echo "$PR_RESPONSE" | jq -r '.prNumber')
